...
-- Toimingud
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `kustutaidJargi` (IN `id` INT)   begin
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `otsi` (IN `taht` VARCHAR(1))   begin
	select * from products;
	select * from products where product_name like Concat(taht + '%');
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `uuendaKategooria` (IN `id` INT, IN `nimi` VARCHAR(20))   BEGIN
	select * from categories;
    update categories set category_name=nimi where category_id=id;
        select * from categories;
END$$
